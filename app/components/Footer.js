                ...
                <div className={"flex flex-col md:flex-row mx-10 mb-10 w-full"}>
                    ...
                    <div className={"flex flex-col w-full h-full md:w-2/3"}>
                        <form action="/" onSubmit={handleSubmit}
                              className={"flex flex-nowrap signup-link w-full py-10 text-xl md:text-3xl lg:text-4xl font-signifier border-b cursor-pointer border-white"}>
                            {!subscribed && <input type="text" placeholder="Email"
                                                   className="p-2 w-0 text-xl font-signifier whitespace-nowrap bg-black placeholder:text-black hover:placeholder:text-white"/>}
                            {!subscribed && <button type="submit"
                                                    className="md:mr-auto min-w-fit pr-[50px] whitespace-nowrap bg-black ml-4">&rarr;</button>}
                            <div className="w-1/2 min-w-fit pr-[50px] whitespace-nowrap">
                                <FadeIn>
                                    <span
                                        className="text-2xl font-signifier">{subscribed ? "Thanks for subscribing!" : "Sign up for strategic insights"}</span>
                                </FadeIn>
                            </div>
                        </form>
                        <div
